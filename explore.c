...
static void analyze_structure(const mxArray *structure_array_ptr) {
  ...
  for (index = 0; index < total_num_of_elements; index++) {
    ...
    for (field_index = 0; field_index < number_of_fields; field_index++) {
      ...
      mexPrintf(".%s\n", field_name);
      field_array_ptr =
          mxGetFieldByNumber(structure_array_ptr, index, field_index);
      ...
      if (starting_row_index == stopping_row_index)
        ...
        for (current_row_index = starting_row_index;
          ...
          mexPrintf("\t(%" FMT_SIZE_T "u,%" FMT_SIZE_T "u) = %g+%g i\n",
                    ir[current_row_index] + 1, col + 1, pc[total].real,
                    pc[total].imag);
          total++;
        }
      }
      ...
      if (starting_row_index == stopping_row_index) {
        ...
        for (current_row_index = starting_row_index;
          ...
          mexPrintf("\t(%" FMT_SIZE_T "u,%" FMT_SIZE_T "u) = %g\n",
                    ir[current_row_index] + 1, col + 1, p[total++]);
        }
      }
    }
  }
  ...
  for (col = 0; col < n; col++) {
    ...
    if (starting_row_index == stopping_row_index)
      ...
      for (current_row_index = starting_row_index;
        ...
        if (mxIsComplex(array_ptr)) {
          mexPrintf("\t(%" FMT_SIZE_T "u,%" FMT_SIZE_T "u) = %g+%g i\n",
                    ir[current_row_index] + 1, col + 1, pr[total], pi[total]);
          total++;
        } else {
          mexPrintf("\t(%" FMT_SIZE_T "u,%" FMT_SIZE_T "u) = %g\n",
...
void display_subscript(const mxArray *array_ptr, mwSize index) {
  ...
  mexPrintf("(");
  subindex = index;
  for (d = number_of_dimensions - 1;;
       d--) { /* loop termination is at the end */
...
/* Check to see if we are on a platform that does not support the compatibility
 * layer. */
  ...
  for (i = 0; i < nrhs; i++) {
    if (mxIsSparse(prhs[i])) {
      mexErrMsgIdAndTxt("MATLAB:explore:NoSparseCompat",
                        "MEX-files compiled on a 64-bit platform that use "
